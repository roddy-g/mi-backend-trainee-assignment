...
def register_advert(db: Session, advert: schemas.Advert):
    db_record = models.Adverts(phrase=advert.phrase, location_id=advert.location_id)
    db.add(db_record)
    db.commit()
    db.refresh(db_record)
    return db.query(models.Adverts).filter(models.Adverts.phrase == advert.phrase).first().id
